...
class pspnet_two_heads(nn.Module):
    ...
    def __init__(self, 
        ...
        super(pspnet_two_heads, self).__init__()
        ...
        self.classification_first_head = nn.Conv2d(512, self.n_classes, 1, 1, 0)
        self.classification_second_head = nn.Conv2d(512, self.n_classes, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        x_first_head = self.classification_first_head(x)
        x_first_head = F.upsample(x_first_head, size=inp_shape, mode='bilinear')

        x_second_head = self.classification_second_head(x)
        x_second_head = F.upsample(x_second_head, size=inp_shape, mode='bilinear')

        # TODO: Add the return values for the aux units (two heads)
        if self.training:
            return x_first_head, x_second_head
        else: # eval mode
            return x_first_head, x_second_head
