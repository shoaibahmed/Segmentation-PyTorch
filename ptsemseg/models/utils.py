...
import os
from ptsemseg.models.deform_conv import DeformConv2D


class conv2D(nn.Module):
    def __init__(self, in_channels, n_filters, k_size, stride, padding, bias=False, dilation=1):
        super(conv2D, self).__init__()

        if dilation > 1:
            self.conv = nn.Conv2d(int(in_channels), int(n_filters), kernel_size=k_size,
...
class deform_conv2D(nn.Module):
    def __init__(self, in_channels, n_filters, k_size,  padding):
        super(deform_conv2D, self).__init__()

        self.offsets = nn.Conv2d(int(in_channels), 18, kernel_size=3, padding=padding)
        self.deform_conv = DeformConv2D(int(in_channels), int(n_filters), kernel_size=k_size, padding=padding)

    def forward(self, inputs):
        offsets = self.offsets(inputs)
        outputs = self.deform_conv(inputs, offsets)
        return outputs


class deform_conv2DBatchNormRelu(nn.Module):
    def __init__(self, in_channels, n_filters, k_size,  padding, with_bn=True):
        super(deform_conv2DBatchNormRelu, self).__init__()

        # self.offsets = nn.Conv2d(int(in_channels), 18, kernel_size=3, padding=padding)
        # self.deform_conv = DeformConv2D(int(in_channels), int(n_filters), kernel_size=k_size, padding=padding)
        deform_conv = deform_conv2D(int(in_channels), int(n_filters), k_size=k_size, padding=padding)

        if with_bn:
            self.cbr_unit = nn.Sequential(deform_conv,
                                          nn.BatchNorm2d(int(n_filters)),
                                          nn.ReLU(inplace=True),)
        else:
            self.cbr_unit = nn.Sequential(deform_conv,
...
class deform_segnetUp3(nn.Module):
    def __init__(self, in_size, out_size):
        super(deform_segnetUp3, self).__init__()
        self.unpool = nn.MaxUnpool2d(2, 2)
        self.conv1 = deform_conv2DBatchNormRelu(in_size, in_size, 3, 1)
        # self.conv2 = conv2DBatchNormRelu(in_size, in_size, 3, 1, 1)
        self.conv2 = deform_conv2DBatchNormRelu(in_size, in_size, 3, 1)
        # self.conv3 = conv2DBatchNormRelu(in_size, out_size, 3, 1, 1)
        self.conv3 = deform_conv2DBatchNormRelu(in_size, out_size, 3, 1)
